...
exports.getQuizDetails = async (req, res) => {
  try {
    ...
    const attemptCount = attempts.length;
    const canAttempt =
      attemptCount < quiz.maxAttempts && quiz.status === "published";
...
exports.startQuizAttempt = async (req, res) => {
  try {
    ...
    console.log("Starting quiz attempt - Quiz details:", {
      id: quiz._id,
      title: quiz.title,
      status: quiz.status,
    ...
    if (quiz.status !== "published") {
      return res.status(400).json({
        success: false,
        message: `Quiz is not available (status: ${quiz.status}). Please contact your coordinator to publish this quiz.`,
      });
    }
    ...
    if (!quiz.isActive) {
      return res.status(400).json({
        success: false,
        message:
          "Quiz is currently deactivated. Please contact your coordinator.",
      });
    }
    ...
    if (now < quiz.startTime || now > quiz.endTime) {
      return res.status(400).json({
        success: false,
        message: `Quiz is not available at this time. Quiz window: ${quiz.startTime} to ${quiz.endTime}`,
      });
    }
