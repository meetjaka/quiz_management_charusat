...
const JSONQuizGenerator = () => {
  ...
  const handleCreateQuiz = async () => {
    ...
    setLoading(true);
    try {
      const totalMarks = parsedData.totalMarks || parsedData.questions.reduce((sum, q) => sum + (q.marks || 1), 0);
      
      // Create quiz with all metadata from JSON
      const quizResponse = await apiClient.post('/coordinator/quizzes', {
        title: parsedData.quizTitle || 'Imported Quiz',
        description: parsedData.description || `Auto-generated from ChatGPT with ${parsedData.questions.length} questions`,
        totalMarks: totalMarks,
        passingMarks: parsedData.passingMarks || Math.ceil(totalMarks * 0.4), // Use JSON value or default 40%
        durationMinutes: parsedData.durationMinutes || parsedData.questions.length * 2, // Use JSON value or default 2 min per question
        maxAttempts: parsedData.maxAttempts || 1,
        shuffleQuestions: parsedData.shuffleQuestions !== undefined ? parsedData.shuffleQuestions : false,
        shuffleOptions: parsedData.shuffleOptions !== undefined ? parsedData.shuffleOptions : false,
        startTime: new Date(),
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Paste the JSON generated by ChatGPT and create your quiz instantly</p>
        </div>
            ...
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 mb-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-medium text-gray-900 mt-1">{parsedData.quizTitle || 'Imported Quiz'}</div>
                </div>
                <div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-md p-6">
          ...
          <ul className="space-y-2 text-sm text-gray-700">
            ...
            <li className="flex gap-2">
              <span>•</span>
              <span>You can edit question details after creating the quiz</span>
            </li>
            <li className="flex gap-2">
              <span>•</span>
              <span>The quiz will be created as "Draft" - you can activate it later</span>
            </li>
            <li className="flex gap-2">
              <span>•</span>
