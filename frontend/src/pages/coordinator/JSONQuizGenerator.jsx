...
const JSONQuizGenerator = () => {
  ...
  const getDifficultyColor = (difficulty) => {
    switch (difficulty) {
      ...
      case 'hard': return 'bg-danger-100 text-danger-700';
  ...
  const getTypeColor = (type) => {
    switch (type) {
      case 'mcq': return 'bg-brand-100 text-brand-700';
      case 'mcq_multiple': return 'bg-indigo-100 text-indigo-700';
      case 'true_false': return 'bg-accent-100 text-accent-700';
      case 'short_answer': return 'bg-warning-100 text-warning-700';
  ...
  return (
    <div className="min-h-screen bg-secondary-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <h2 className="text-3xl font-bold text-secondary-900 flex items-center gap-2">
            <Sparkles className="w-8 h-8 text-success-600" />
            Generate Quiz from ChatGPT JSON
          </h2>
          <p className="text-secondary-600 mt-2">Paste the JSON generated by ChatGPT to import questions (quiz details are collected separately)</p>
        </div>
        ...
        <div className="bg-brand-50 border border-brand-200 rounded-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-brand-700 mt-1">Follow our step-by-step guide to use ChatGPT</p>
            </div>
            <a
        ...
        <div className="bg-white border border-secondary-200 rounded-lg shadow-card-hover p-6 mb-6">
          <h3 className="text-lg font-bold text-secondary-900 mb-3">📝 Step 1: Paste JSON from ChatGPT</h3>
          <textarea
            value={jsonInput}
          ...
          <button
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 bg-success-600 text-white py-3 rounded-lg hover:bg-success-700 disabled:bg-secondary-400 disabled:cursor-not-allowed font-medium transition-colors"
          >
          ...
          <div className="bg-white border border-secondary-200 rounded-lg shadow-card-hover p-6 mb-6">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 mb-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <div className="font-medium text-secondary-900 mt-1">{parsedData.questions.length}</div>
                </div>
                <div>
                  <span className="text-secondary-600">Total Marks:</span>
                  <div className="font-medium text-secondary-900 mt-1">
            ...
            <div className="max-h-96 overflow-y-auto space-y-4 mb-6">
              {parsedData.questions.map((q, idx) => (
                <div key={idx} className="border border-secondary-200 rounded-lg p-4 hover:bg-secondary-50 transition-colors">
                  <div className="flex justify-between items-start mb-2">
                    <div className="font-medium text-secondary-900 flex-1">
                      ...
                      {q.options.map((opt, optIdx) => (
                        <div
                          ...
                          className={`pl-4 py-1 rounded ${
                            opt.isCorrect ? 'text-success-700 font-medium bg-success-50' : 'text-secondary-700'
                          }`}
                        >
                  ...
                  {q.questionType === 'short_answer' && (
                    <div className="mt-2 text-sm bg-success-50 text-success-700 p-2 rounded">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex-1 flex items-center justify-center gap-2 bg-brand-600 text-white py-3 rounded-lg hover:bg-brand-700 disabled:bg-secondary-400 disabled:cursor-not-allowed font-medium text-lg transition-colors"
              >
                {loading ? (
                  <>
        ...
        <div className="bg-white border border-secondary-200 rounded-lg shadow-card-hover p-6">
          <h3 className="font-bold text-secondary-900 mb-3">💡 Tips:</h3>
