...
const AdminDashboard = () => {
  ...
  const { overview, recentQuizzes, recentAttempts, departmentStats } = analytics || {};
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1">Monitor and manage your quiz management system</p>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <h2 className="text-lg font-bold text-gray-900">Recent Quiz Attempts</h2>
                <Link to="/admin/analytics" className="text-sm font-medium text-blue-600 hover:text-blue-700 flex items-center gap-1">
                      ...
                      recentAttempts.map((attempt) => (
                        <tr key={attempt._id} className="hover:bg-gray-50/50 transition-colors">
                          <td className="px-6 py-4">
                            <div className="font-medium text-gray-900">{attempt.studentId?.name || "Unknown"}</div>
                            <div className="text-xs text-gray-500">{attempt.studentId?.enrollmentNumber || "N/A"}</div>
                          </td>
                          <td className="px-6 py-4 text-gray-600">{attempt.quizId?.title || "Unknown Quiz"}</td>
                          <td className="px-6 py-4 font-semibold text-gray-900">
                            {attempt.totalScore !== undefined ? `${attempt.totalScore}%` : "N/A"}
                          </td>
                          <td className="px-6 py-4">
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-6">Department Statistics</h2>
                  ...
                  departmentStats.map((dept, index) => (
                    <div key={index}>
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${(dept.count / (overview?.totalStudents || 1)) * 100}%` }}
          ...
          <div className="space-y-8">
            ...
            <motion.div variants={itemVariants} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                ...
                <Link to="/admin/quizzes" className="text-xs font-medium text-gray-500 hover:text-blue-600">View All</Link>
              </div>
              <div className="divide-y divide-gray-100">
                {recentQuizzes && recentQuizzes.length > 0 ? (
                  recentQuizzes.map((quiz) => (
                    <div key={quiz._id} className="p-4 hover:bg-gray-50 transition-colors">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className="text-sm font-semibold text-gray-900 line-clamp-1">{quiz.title}</h3>
                      ...
                      <p className="text-xs text-gray-500 mb-2">By {quiz.createdBy?.name || "Unknown"}</p>
                      <div className="flex items-center gap-3 text-xs text-gray-400">
                        <span className="flex items-center gap-1"><School className="w-3 h-3" /> {quiz.questions?.length || 0} Qs</span>
                        <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {quiz.duration}m</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="p-8 text-center text-sm text-gray-500">No quizzes created yet</div>
                )}
              </div>
            </motion.div>
...
const StatCard = ({ title, value, icon: Icon, color, bg, link }) => {
  ...
  return link ? <Link to={link} className="block h-full">{content}</Link> : <div className="h-full">{content}</div>;
};

const QuickActionButton = ({ title, icon: Icon, link, color, bg, borderColor }) => {
  return (
    <Link
      to={link}
    ...
    >
      <div className={`p-3 rounded-full mb-3 group-hover:scale-110 transition-transform ${bg}`}>
